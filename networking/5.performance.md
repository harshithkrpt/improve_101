...
Here’s the core mechanism, stripped to first principles.
...
Segments sent: `1 → 2 → 3 → 4 → 5`
Segment **2** gets lost on the network.

What happens?

Segment 3, 4, and 5 may actually arrive just fine. TCP sees them… and then **locks them in a buffer**. The application gets **nothing** until segment 2 is retransmitted and arrives.

That waiting—where later data is blocked by an earlier missing piece—is **head-of-line blocking**.
...
* Interactive apps feeling “sticky” on bad networks
...
That choice powered the internet for decades—but modern real-time apps keep poking at its limits.

Once you see HoL blocking as a design decision rather than a flaw, the evolution from TCP → QUIC starts to feel inevitable rather than trendy.
...
## Proxy (Forward Proxy) — the client’s bodyguard
...
* **Source IP hiding**: the server only sees the proxy’s IP.
* **Protocol awareness**: often HTTP-aware, sometimes raw TCP.
* **TLS behavior**:

  * CONNECT tunnel → proxy can’t see inside (just shovels bytes).
  * TLS termination → proxy becomes a man-in-the-middle (with certs).
...
## Reverse Proxy — the server’s public face
...
* **Connection multiplexing**: one client connection → many backend hops.
...
Mental model: *The proxy is the server’s mask.*

---

## Load Balancer — traffic physics, not business logic
...
## API Gateway — protocol translator with opinions
...
* Authentication (JWT, OAuth → verified *before* forwarding)
* Rate limiting (token buckets tied to IP / API key)
* Request shaping (one client call → many backend calls)
...
## Sidecar Proxy — networking outsourced from your app
...
From the packet’s view:

* App thinks it’s talking to `localhost`.
* iptables / routing rules silently divert traffic to the sidecar.
* Sidecar opens outbound connections on the app’s behalf.

Why this is radical:

* The application no longer “does networking.”
...
* Extra hops
* More connections
* Debugging moves from code → packet paths

Mental model: *The sidecar is the service’s nervous system.*
...
Once you see them as *TCP conversations with opinions*, the fog lifts—and the packets start telling a very coherent story.

Load balancing is one of those ideas that sounds simple—“spread traffic”—until you look at *where* in the networking stack the decision is made. That “where” changes everything.

---

## Layer 4 (Transport Layer) Load Balancing

*(TCP / UDP aware, application ignorant)*

![Image](https://www.loadbalancer.org/blog/content/images/2020/05/concept-dsr-1.png)

![Image](https://media.geeksforgeeks.org/wp-content/uploads/20240626162952/Load-Balancing-TCP-Traffic.webp)

![Image](https://developers.cloudflare.com/_astro/lb-ref-arch-28-ALT.Dwf-s8s__1NmnXS.svg)

Layer 4 load balancers live at the **transport layer**. They see IP addresses, ports, and protocol flags. That’s it. No HTTP headers. No URLs. No cookies. Just packets with ambitions.

**How it works**
A client connects to `VIP:PORT`.
The load balancer picks a backend server based on metadata like:

* source IP
* destination port
* protocol (TCP/UDP)
* sometimes a hash of the 5-tuple (src IP, src port, dst IP, dst port, protocol)

Once chosen, the balancer forwards packets back and forth, often with **NAT** or **DSR (Direct Server Return)**.

**Key properties**

* Blazing fast: minimal inspection, often kernel or hardware accelerated
* Connection-based: once a TCP connection is mapped, it sticks
* Protocol-agnostic: HTTP, HTTPS, gRPC, MySQL, Redis—doesn’t care

**Typical algorithms**

* Round-robin
* Least connections
* Hash-based (source IP hash)

**Strengths**

* Very low latency
* Scales to absurd traffic levels
* Works for *any* TCP/UDP protocol

**Limitations**

* No idea what the request *means*
* Can’t route based on `/api/v1/users`
* TLS is opaque unless you terminate it elsewhere

**Mental model**
Think of a nightclub bouncer who only checks your wristband color and sends you to a room. No clue why you’re there.

---

## Layer 7 (Application Layer) Load Balancing

*(HTTP-aware, meaning-curious)*

![Image](https://freeloadbalancer.com/images/freeloadbalancerlibraries/default-album/graphics/freeloadbalancer-layer-4-layer-7-mobile.svg?sfvrsn=6c04fa85_4)
...
Layer 7 load balancers operate where requests have *semantics*. They understand HTTP, HTTPS, headers, paths, methods, cookies, sometimes even JSON payloads.

**How it works**
The load balancer **terminates the client connection**, parses the request, makes a decision, then creates a *new* connection to a backend.

Routing decisions can be based on:

* URL path (`/images` → cache servers)
* Host header (`api.example.com`)
* HTTP method (`POST` vs `GET`)
* Cookies / session IDs
* Headers (auth, region, device type)

**Key properties**

* Acts like a reverse proxy
* Often handles TLS termination
* Can modify requests and responses

**Strengths**

* Smart routing
* Enables microservices and API gateways
* Supports retries, rate limiting, auth, caching

**Limitations**

* More CPU and memory
* Higher latency than L4
* Protocol-specific (mostly HTTP/gRPC)

**Mental model**
A hotel concierge who reads your reservation, asks why you’re visiting, and sends you to exactly the right floor.

---

## Side-by-Side Intuition (no buzzwords)

* **L4 asks:** “Where did this packet come from?”

* **L7 asks:** “What does this request want?”

* **L4 balances connections**

* **L7 balances requests**

* **L4 is fast and dumb (by design)**

* **L7 is slower and clever**

---

## Real-World Architecture (what actually happens)

In production, you almost never choose *only one*.

A common pattern:

* **L4 at the edge**
  Handles raw scale, TCP fan-out, DoS resistance
* **L7 behind it**
  Does TLS termination, routing, auth, observability

Packets sprint through L4. Requests stop and think at L7.

---

## A subtle but important detail

With **HTTP/2 or HTTP/3**, a single TCP (or QUIC) connection can carry *many* requests concurrently.

* L4 sees **one connection**
* L7 sees **many streams**

That’s why modern systems lean heavily on L7 when traffic patterns get complex.

---

## A useful rule of thumb

If your routing logic can be expressed as:

> “This IP/port should go there”

You want **Layer 4**.

If it sounds like:

> “Requests that *look like this* should go there”

You want **Layer 7**.

And if you’re running something serious at scale, you probably want both—because the network stack, much like the universe, prefers layered realities over single truths.
